...
import 'package:flutter_firebase_goals/models/goal.dart';
import 'package:flutter_firebase_goals/models/user_goals.dart';
import 'package:flutter_firebase_goals/repositories/repository.dart';
import 'package:flutter_firebase_goals/utils/strings.dart';
...
class GoalsBloc {
  ...
  final _validateMessage = StreamTransformer<String, String>.fromHandlers(
    ...
    if (goalMessage.length > 10) {
      ...
      sink.addError("Message not long enough.");
    }
  });
  ...
  final _validateName = StreamTransformer<String, String>.fromHandlers(
    ...
    if (RegExp(r'[!@#<>?":_`~;[\]\\|=+)(*&^%0-9-]').hasMatch(name)) {
      sink.addError("Goal name has been used.");
    } else {
      sink.add(name);
    }
  });
